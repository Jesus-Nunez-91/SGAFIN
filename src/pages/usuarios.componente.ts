                     ...
                     <div class="group">
                        ...
                        <div class="relative">
                            ...
                            <select [(ngModel)]="editUser.rol" class="w-full pl-10 pr-4 py-3 rounded-xl bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 focus:bg-white dark:focus:bg-gray-600 focus:border-uah-orange focus:ring-4 focus:ring-uah-orange/10 dark:text-white transition-all text-sm font-bold outline-none appearance-none">
                                ...
                                <option value="Admin_Labs">Encargado de Lab</option>
                                <option value="Admin_Acade">Coordinador Académico</option>
                      ...
                      <div class="group">
                       ...
                       <!-- Resumen de Permisos Asignados Automáticamente -->
                       <div class="mt-6 bg-gray-50/50 dark:bg-gray-900/50 p-6 rounded-2xl border border-gray-100 dark:border-gray-700">
                           <h4 class="text-xs font-black text-uah-blue dark:text-white uppercase tracking-[0.2em] mb-4 border-l-4 border-uah-orange pl-3">Permisos del Rol</h4>
                           
                           <div class="space-y-3">
                               <!-- Permiso: Total -->
                               @if (editUser.rol === 'SuperUser') {
                                   <div class="flex items-center gap-3 p-3 rounded-xl bg-red-500/10 border border-red-500/20 text-red-600 dark:text-red-400">
                                       <i class="bi bi-shield-fill-check text-lg"></i>
                                       <div>
                                           <div class="text-xs font-black uppercase tracking-wider">Acceso Total (Administrador)</div>
                                           <div class="text-[10px] opacity-85 mt-0.5">Control completo del sistema, configuración, auditoría y gestión de todos los usuarios.</div>
                                       </div>
                                   </div>
                               }
                               
                               <!-- Permiso: Encargado de Lab -->
                               @if (editUser.rol === 'Admin_Labs') {
                                   <div class="flex items-center gap-3 p-3 rounded-xl bg-blue-500/10 border border-blue-500/20 text-blue-600 dark:text-blue-400">
                                       <i class="bi bi-cpu text-lg"></i>
                                       <div>
                                           <div class="text-xs font-black uppercase tracking-wider">Gestión de Laboratorios e Inventario</div>
                                           <div class="text-[10px] opacity-85 mt-0.5">Aprobación de reservas, control de stock crítico, equipos, salas y bitácoras. Sin acceso a gestión de usuarios.</div>
                                       </div>
                                   </div>
                               }
                               
                               <!-- Permiso: Coordinador Académico -->
                               @if (editUser.rol === 'Admin_Acade') {
                                   <div class="flex items-center gap-3 p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400">
                                       <i class="bi bi-calendar-event text-lg"></i>
                                       <div>
                                           <div class="text-xs font-black uppercase tracking-wider">Coordinación de Horarios</div>
                                           <div class="text-[10px] opacity-85 mt-0.5">Administración y asignación de bloques horarios, asignaturas de docencia y monitoreo de salas en tiempo real.</div>
                                       </div>
                                   </div>
                               }
                               
                               <!-- Permiso: Docente / Alumno -->
                               @if (editUser.rol === 'Docente' || editUser.rol === 'Alumno') {
                                   <div class="flex items-center gap-3 p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-600 dark:text-amber-400">
                                       <i class="bi bi-journal-bookmark-fill text-lg"></i>
                                       <div>
                                           <div class="text-xs font-black uppercase tracking-wider">Solicitud de Reservas</div>
                                           <div class="text-[10px] opacity-85 mt-0.5">Permiso para realizar reservas de salas/materiales, ver estado de solicitudes personales y consultar horarios vigentes.</div>
                                       </div>
                                   </div>
                               }
                           </div>
                                  ...
                                  <td class="p-5 pl-6">
                                     <div class="flex items-center gap-4">
                                         <div class="w-10 h-10 rounded-full bg-gray-100 dark:bg-gray-700 text-uah-blue font-black flex items-center justify-center shadow-sm group-hover:bg-uah-blue group-hover:text-white transition-all">
                                             {{ (u.nombreCompleto || u.correo || '?').charAt(0).toUpperCase() }}
                                         </div>
                                         <div>
...
export class ComponenteUsuarios {
   @ViewChild('fileInput') fileInput!: ElementRef;
   data = inject(DataService);
   editUser: Partial<User> = { rol: 'Alumno', permisos: {} };
   searchTerm = signal('');
   ...
   reset() { 
      this.editUser = { rol: 'Alumno', permisos: {} }; 
   }
   ...
   edit(u: User) {
      this.editUser = { ...u, password: '', permisos: u.permisos || {} };
   ...
   async save() {
      ...
      // Asignación automática de permisos según el rol elegido
      const rol = this.editUser.rol;
      if (rol === 'SuperUser') {
         this.editUser.permisos = {};
      } else if (rol === 'Admin_Labs') {
         this.editUser.permisos = {
            'Welcome': 'Editor',
            'Dashboard': 'Editor',
            'Laboratorio': 'Editor',
            'Horarios Academicos': 'Solo Vista',
            'Salas y Labs': 'Editor',
            'Gestion de Solicitudes': 'Editor',
            'Ayuda & Soporte': 'Editor',
            'Wiki': 'Editor',
            'Compras': 'Editor',
            'Mantencion': 'Editor',
            'Bitagora': 'Editor',
            'Proyectos': 'Editor',
            'Auditoria': 'Editor',
            'Prestamo Equipos': 'Editor'
         };
      } else if (rol === 'Admin_Acade') {
         this.editUser.permisos = {
            'Welcome': 'Solo Vista',
            'Dashboard': 'Solo Vista',
            'Horarios Academicos': 'Editor'
         };
      } else if (rol === 'Docente' || rol === 'Alumno') {
         this.editUser.permisos = {
            'Welcome': 'Solo Vista',
            'Dashboard': 'Solo Vista',
            'Laboratorio': 'Solo Reserva',
            'Horarios Academicos': 'Solo Vista'
         };
      }
      ...
      try {
         ...
         Swal.fire({
            ...
            text: err.message || 'Ocurrió un error inesperado al guardar el usuario.',
            customClass: {
              popup: 'uah-premium-popup',
              title: 'uah-premium-title',
              confirmButton: 'uah-premium-confirm'
