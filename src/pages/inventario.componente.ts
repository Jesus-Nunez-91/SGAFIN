              ...
              <div class="text-center mb-4">
                  <div class="w-full h-36 bg-gray-50 dark:bg-gray-900 rounded-xl flex items-center justify-center overflow-hidden border-2 border-dashed border-gray-300 dark:border-gray-600 relative cursor-pointer hover:bg-blue-50 dark:hover:bg-blue-900/20 hover:border-blue-300 transition-all group">
                      @if (editItem.imagenUrl) {
                          <img [src]="editItem.imagenUrl" class="w-full h-full object-cover" [alt]="'Imagen de previsualización para ' + (editItem.marca || 'Recurso')">
                        ...
                        <tr [class]="isLowStock ? 'bg-red-50/50 dark:bg-red-900/10 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors border-l-4 border-red-400' : 'hover:bg-blue-50/40 dark:hover:bg-blue-900/10 transition-colors group border-l-4 border-transparent'"
                          ...
                          <td class="p-4 text-center">
                             <div class="flex items-center justify-center gap-2">
                                <button (click)="openDetails(item)" title="Ver Detalle Técnicos" class="text-gray-400 hover:text-uah-blue dark:hover:text-blue-300 hover:bg-blue-50 dark:hover:bg-gray-700 w-8 h-8 rounded-full transition-colors flex items-center justify-center">
                                   <i class="bi bi-info-circle-fill text-lg"></i>
                                </button>
                                
                                <button (click)="showItemAvailabilityModal(item)" title="Ver Disponibilidad por Horarios" class="text-[#f06427] hover:bg-orange-50 dark:hover:bg-gray-700 w-8 h-8 rounded-full transition-colors flex items-center justify-center">
                                ...
                                @if (item.status === 'Disponible' && stock > 0) {
                                    <div class="flex items-center border border-gray-300 dark:border-gray-600 rounded-lg overflow-hidden bg-white dark:bg-gray-700 shadow-sm ml-2 h-8">
                                       ...
                                       @if (isAdmin()) {
                                          <button (click)="requestSingleItem(item, +qtyInput.value)" class="bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-200 hover:bg-uah-orange hover:text-white border-l border-gray-300 dark:border-gray-500 px-3 h-full font-black text-[10px] transition-all uppercase tracking-widest">
                                             SOLICITAR
                                          </button>
                                       } @else {
                                          <button (click)="toggleSelection(item, +qtyInput.value)" 
                                             ...
                                             {{ isSelected(item.id) ? 'QUITAR' : 'SOLICITAR' }}
                                          </button>
                                       }
                                    </div>
                                }
...
export class ComponenteInventario {
  ...
  /** Exporta los items filtrados a un archivo CSV. */
  exportData() {
    this.data.downloadCSV(this.filteredItems(), `Inventario_${this.labName}_${this.inventoryMode()}`);
  }
  ...
  getAvailableStock(item: InventoryItem): number {
    const selectedDate = this.resDate();
    const selectedBlock = this.resBlock();
    ...
    const normSelDate = normalizeDate(selectedDate);

    const reserved = this.data.reservations().filter(r =>
      !r.rechazada &&
      (r.equipoId === item.id) &&
      (item.esFungible ? !r.aprobada : (normalizeDate(r.fecha) === normSelDate && r.bloque === selectedBlock))
    ).reduce((sum, r) => sum + (Number(r.cantidad) || 1), 0);

    return Math.max(0, item.stockActual - reserved - (item.stockDefectuoso || 0));
  }

  showItemAvailabilityModal(item: InventoryItem) {
    const timeBlocks = [
      '08:30 - 09:50',
      '10:00 - 11:20',
      '11:30 - 12:50',
    ...
    const rows = timeBlocks.map(block => {
      const reserved = this.data.reservations().filter(r =>
        !r.rechazada &&
        (r.equipoId === item.id) &&
        (item.esFungible ? !r.aprobada : (r.fecha === this.resDate() && r.bloque === block))
      ).reduce((sum, r) => sum + r.cantidad, 0);

      const avail = Math.max(0, item.stockActual - reserved - (item.stockDefectuoso || 0));
      const statusBadge = avail > 0
        ? `<span class="px-2 py-1 rounded bg-emerald-100 text-emerald-800 font-bold text-xs">${avail} Disponibles</span>`
        : `<span class="px-2 py-1 rounded bg-red-100 text-red-800 font-bold text-xs">Sin Stock</span>`;

      return `
        <tr class="border-b border-gray-100 text-left">
          <td class="py-2.5 px-3 font-bold text-xs text-gray-700">${block}</td>
          <td class="py-2.5 px-3 text-xs text-gray-500">${reserved} reservados</td>
    ...
    Swal.fire({
      title: `Disponibilidad de ${item.marca} ${item.modelo}`,
      html: `
        <div class="text-left mb-3">
          <p class="text-xs text-gray-500 font-bold uppercase">Fecha de Consulta: <span class="text-[#003366] font-black">${this.resDate()}</span></p>
          <p class="text-xs text-gray-400">Stock Total: <b>${item.stockActual}</b> | Defectuosos: <b>${item.stockDefectuoso || 0}</b></p>
        </div>
        <div class="overflow-x-auto border border-gray-200 rounded-xl">
          <table class="w-full text-sm">
            <thead class="bg-gray-100 text-gray-600 font-black uppercase text-[10px]">
              <tr>
                <th class="p-2.5 text-left">Bloque Horario</th>
                <th class="p-2.5 text-left">Ocupados</th>
  ...
  async requestSingleItem(item: InventoryItem, qty: number) {
    const date = this.resDate();
    const block = this.resBlock();
    if (!date || !block) {
      Swal.fire('Atención', 'Por favor selecciona la fecha y bloque horario en los filtros superiores.', 'warning');
      return;
    }

    const leadTimeError = this.getLeadTimeError();
    if (leadTimeError) {
      Swal.fire('Restricción de Antelación', leadTimeError, 'warning');
      return;
    }

    await this.data.createReservation(item, date, block, qty);

    Swal.fire({
      icon: 'success',
      title: 'SOLICITUD ENVIADA',
      text: `Has solicitado ${qty} unidad(es) de ${item.marca} ${item.modelo} para el ${date} en el bloque ${block}. El encargado revisará tu solicitud a la brevedad.`,
      confirmButtonColor: '#003366'
    });
  }
  ...
  makeReservation() {
    ...
    this.executeReservation(date, block);
  }

  private executeReservation(date: string, block: string) {
  ...
  applyArrival() {
    const qty = Number(this.editItem.cantidadLlegada) || 0;
    if (qty > 0) {
      this.editItem.stockActual = (Number(this.editItem.stockActual) || 0) + qty;
      this.editItem.cantidadLlegada = 0; // Reiniciar input después de sumar
      Swal.fire({ icon: 'info', toast: true, position: 'top-end', title: `Sumado ${qty} al stock`, timer: 1500, showConfirmButton: false });
    } else {
      Swal.fire({ icon: 'warning', toast: true, position: 'top-end', title: 'Ingrese un valor mayor a 0', timer: 1500, showConfirmButton: false });
    }
  }
  ...
  exportSelectedItems() {
    ...
    this.data.downloadCSV(mapped, `Inventario_SGA_${this.labName}_${this.inventoryMode()}`);
  }
}
