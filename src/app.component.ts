...
@Component({
    ...
    template: `
@if (authService.currentUser() && !isWelcomePage() && !isLoginPage()) {
      ...
      <aside [class.translate-x-0]="showMobileMenu()" [class.-translate-x-full]="!showMobileMenu()" 
          ...
          <div class="h-32 flex items-center justify-between px-6 border-b border-[#f06427]/20 bg-black">
              <a routerLink="/areas" (click)="closeMobileMenu()" class="flex items-center gap-4 group cursor-pointer">
                    <div class="relative">
                        <img src="assets/images/UAH-avatar_FIN.png" class="h-14 w-14 lg:h-12 lg:w-12 xl:h-16 xl:w-16 shadow-2xl relative z-10 object-contain rounded-full border-2 border-[#f06427] hover:scale-110 transition-transform duration-300" alt="Logo UAH">
          ...
          <nav class="flex-1 overflow-y-auto py-8 flex flex-col gap-1.5 px-4 scrollbar-hide">
              @if (isSuperUser() || hasPermission('Dashboard', 'ver')) {
                  <a (click)="closeMobileMenu()" routerLink="/panel-principal" routerLinkActive="bg-[#f06427] text-white shadow-lg shadow-[#f06427]/20" 
              ...
              @if (isSuperUser() || hasPermission('Laboratorio', 'ver')) {
                 <a (click)="closeMobileMenu()" routerLink="/laboratorios" routerLinkActive="bg-[#f06427] text-white shadow-lg shadow-[#f06427]/20" 
                    class="flex items-center gap-4 px-5 py-3.5 rounded-2xl text-gray-400 hover:text-white hover:bg-white/5 transition-all group font-bold tracking-tight">
              ...
              @if (isSuperUser() || hasPermission('Horarios Academicos', 'ver')) {
                 <a (click)="closeMobileMenu()" routerLink="/horario-academico" routerLinkActive="bg-[#f06427] text-white shadow-lg shadow-[#f06427]/20" 
                    class="flex items-center gap-4 px-5 py-3.5 rounded-2xl text-gray-400 hover:text-white hover:bg-white/5 transition-all group font-bold tracking-tight">
              ...
              @if (isSuperUser()) {
                 <a (click)="closeMobileMenu()" routerLink="/usuarios" routerLinkActive="bg-[#f06427] text-white shadow-lg shadow-[#f06427]/20" 
                    class="flex items-center gap-4 px-5 py-3.5 rounded-2xl text-gray-400 hover:text-white hover:bg-white/5 transition-all group font-bold tracking-tight">
                    <i class="bi bi-people-fill text-xl group-hover:scale-110 transition-transform"></i>
                    <span class="lg:hidden xl:inline text-sm">Gestión de Usuarios</span>
                </a>
              }
          </nav>
          ...
          <div class="flex-1 overflow-y-auto overflow-x-hidden scroll-smooth custom-scrollbar bg-gray-50 dark:bg-[#050505]">
              ...
              <footer class="mt-20 border-t border-gray-200 dark:border-white/5 bg-white dark:bg-black p-10 lg:p-20 relative overflow-hidden">
                  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-20">
                      <!-- Col 1: Logo & Info -->
                      <div class="space-y-8">
                          <img [src]="authService.darkMode() ? 'assets/images/UAH-logo_FIN-blanco.png' : 'assets/images/UAH-logo_FIN-negro.png'" 
                               class="h-10 w-auto rounded-lg shadow-sm" alt="Logo Institucional Universidad Alberto Hurtado">
...
export class AppComponent {
  ...
  isWelcomePage = computed(() => this.currentUrl().includes('/bienvenida'));
  isLoginPage = computed(() => this.currentUrl().includes('/inicio-sesion'));
    ...
    checkPermissions() {
        ...
        // 3. Zonas de Consulta (Laboratorios, Horarios, Inventario)
        const labZone = (url.includes('laboratorios') || url.includes('inventario')) && !this.isLabAdmin() && !this.hasPermission('Laboratorio', 'ver');
        const scheduleZone = url.includes('horario-academico') && !this.isAcadeAdmin() && !this.hasPermission('Horarios Academicos', 'ver');

        if (labZone || scheduleZone) {
            this.router.navigate(['/bienvenida']);
            return;
        }
    }

    getDefaultPermisosForRol(rol: string): any {
        if (rol === 'SuperUser') return {};
        if (rol === 'Admin_Labs') {
            return {
                'Welcome': 'Editor',
                'Dashboard': 'Editor',
    ...
    hasPermission(module: string, action: 'ver' | 'editar' | 'reservar'): boolean {
        ...
        const permisos = user.permisos || this.getDefaultPermisosForRol(user.rol);
        if (!permisos) return false;

        const level = permisos[module];
