...
export class DataService {
  ...
  constructor() {
    ...
    if (this.currentUser()?.rol === 'SuperUser') {
      this.fetchAuditLogs();
    }
    // this.fetchProjects();
    // this.fetchWiki();
  ...
  async addItem(item: any) {
    if (!this.token()) return;
    try {
      const res = await fetch(this.baseUrl + '/api/laboratorios/articulos', { credentials: 'include', method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${this.token()}`
        },
  ...
  async checkIn(id: number) {
    try {
      const res = await fetch(this.baseUrl + `/api/reservas/${id}/check-in`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${this.token()}` }
      });
      if (res.ok) {
  ...
  async checkOut(id: number) {
    try {
      const res = await fetch(this.baseUrl + `/api/reservas/${id}/check-out`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${this.token()}` }
      });
      if (res.ok) {
  ...
  async updateItem(id: number, item: any) {
    if (!this.token()) return;
    try {
      const res = await fetch(this.baseUrl + `/api/laboratorios/articulos/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${this.token()}`
  ...
  async deleteLaboratoryReservation(id: number): Promise<boolean> {
    if (!this.token()) return false;
    try {
      const res = await fetch(this.baseUrl + `/api/reservas/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${this.token()}` }
      });
      if (res.ok) {
  ...
  async addBulkItems(items: any[]) {
    if (!this.token()) return false;
    try {
      const res = await fetch(this.baseUrl + '/api/laboratorios/articulos/bulk', { credentials: 'include', method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${this.token()}`
        },
  ...
  async clearAllInventory() {
    if (!this.token()) return;
    try {
      const res = await fetch(this.baseUrl + '/api/laboratorios/articulos/mass/clear', { credentials: 'include', method: 'DELETE',
        headers: { 'Authorization': `Bearer ${this.token()}` }
      });
      if (res.ok) await this.fetchInventory();
      return res.ok;
  ...
  async createReservation(item: InventoryItem, fecha: string, bloque: string, cantidad: number) {
    ...
    const resPayload: any = {
      ...
      nombreSolicitante: user?.nombreCompleto || 'Usuario',
      rutSolicitante: user?.rut || '12345678-9',
      emailSolicitante: user?.correo || 'usuario@uah.cl',
      tipoUsuario: user?.rol || 'Alumno',
      aprobada: false,
      rechazada: false,
      solicitanteId: user?.id,
      user: user?.correo || 'usuario@uah.cl',
      userRole: user?.rol || 'Alumno',
      tipoItem: item.tipoInventario || 'EQUIPO',
      detalle: `${item.marca} ${item.modelo}`,
      createdAt: new Date().toISOString()
    };

    // Optimistic local signal update
    this.reservations.update(list => [resPayload, ...list]);
    this.save();

    if (!this.token()) return;
    try {
      const { id, ...sendPayload } = resPayload;
      const res = await fetch(this.baseUrl + '/api/reservas', { credentials: 'include', method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${this.token()}`
        },
  ...
  async updateReservationStatus(id: number, status: string, payload?: any) {
    ...
    if (!this.token()) return;
    try {
      const res = await fetch(this.baseUrl + `/api/reservas/${id}/estado`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${this.token()}`
  ...
  async fetchAuditLogs() {
    if (!this.token()) return;
    try {
      const res = await fetch(this.baseUrl + '/api/auditoria', { credentials: 'include', headers: { 'Authorization': `Bearer ${this.token()}` }
      });
      if (res.ok) {
        const data = await res.json();
  ...
  // --- TICKETS DE SOPORTE ---
  createTicket(subject: string, message: string, laboratory?: string, priority?: string, category?: string) {
    const user = this.currentUser();
  ...
  async updateUser(id: number, user: any) {
    if (!this.token()) return false;
    try {
      const res = await fetch(this.baseUrl + `/api/usuarios/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${this.token()}`
  ...
  async fetchSchedules() {
    ...
    try {
      const res = await this.apiFetch('/api/horarios');
      if (res.ok) {
        const data = await res.json();
  ...
  async addBulkSchedules(schedules: any[]) {
    if (!this.token()) return;
    try {
      const res = await this.apiFetch('/api/horarios/bulk', {
        method: 'POST',
  ...
  async fetchUnifiedRequests() {
    this.unifiedRequests.set([]); // Módulo removido
  }

  async deleteSchedule(id: number) {
    if (!this.token()) return;
    try {
      const res = await this.apiFetch(`/api/horarios/${id}`, {
        method: 'DELETE'
      });
      if (res.ok) {
        await this.fetchSchedules();
  ...
  async addBulkUsers(users: any[]) {
    if (!this.token()) return;
    try {
      const res = await this.apiFetch('/api/usuarios/bulk', {
        method: 'POST',
  ...
  async login(correo: string, pass: string, recaptchaToken?: string): Promise<boolean> {
    try {
      const res = await this.apiFetch('/api/auth/login', {
        method: 'POST',
        body: JSON.stringify({ correo, contrasena: pass, recaptchaToken })
      });
  ...
  async fetchInventory() {
    if (!this.token()) return;
    try {
      const res = await this.apiFetch('/api/laboratorios/articulos');
      if (res.ok) {
        const data = await res.json();
  ...
  async deleteLabSchedules(lab: string): Promise<boolean> {
    if (!this.token()) return false;
    try {
      const res = await fetch(this.baseUrl + `/api/horarios/lab/${lab}`, { credentials: 'include', method: 'DELETE',
        headers: { 'Authorization': `Bearer ${this.token()}` }
      });
      if (res.ok) {
        await this.fetchSchedules();
